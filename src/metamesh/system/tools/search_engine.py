...
config = Config()
from metamesh.system.tools import SearchEngineType


class SearchEngine:
    """
    TODO: Integrate Google Search with reverse proxy
    Note: Google requires Proxifier or similar global proxy here
...
def safe_google_results(results: str | list) -> str:
    ...
    if isinstance(results, list):
        safe_message = json.dumps(
            # FIXME: # .encode("utf-8", "ignore") was removed here, but AutoGPT has it, which is strange
